...
def main():
    ...
    table_name = os.getenv('DYNAMODB_TABLE_1_TEST')
    bucket_name = os.getenv('S3_BUCKET_TEST')
    file_format = os.getenv('FILE_FORMAT', 'csv')
    role = os.getenv('AWS_ROLE_ARN')
    ingest_type = 'ingest-service-1'
    glue_database = f"glue_database_{ingest_type}_{table_name}_test"
    glue_crawler_name = f"crawler_{ingest_type}_{table_name}_test"
    
    if not table_name or not bucket_name:
        logger.error("Error: DYNAMODB_TABLE_1_TEST y S3_BUCKET_TEST son obligatorios.")
        return
